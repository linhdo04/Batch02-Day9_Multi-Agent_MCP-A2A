...
## Improve Agent Day 08 bằng Supervisor - Workers

Project giữ lại toàn bộ RAG pipeline của Day 08 và bổ sung một lớp
multi-agent orchestration bằng LangGraph:

```mermaid
flowchart TD
    U[User Question] --> S[Supervisor]
    S -->|Send API| L[Legal Research Worker]
    S -->|Send API| N[News Research Worker]
    S -->|Send API| V[Verification Worker]
    L --> A[Synthesizer]
    N --> A
    V --> A
    A --> R[Final Answer + Citations]
```
...
| Agent | Trách nhiệm |
|---|---|
| Supervisor | Phân rã câu hỏi và tạo kế hoạch cho các workers |
| Legal Research Worker | Tra cứu corpus văn bản pháp luật |
| News Research Worker | Tra cứu corpus tin tức |
| Verification Worker | Kiểm tra độc lập độ bao phủ và độ đa dạng nguồn |
| Synthesizer | Gom kết quả, loại trùng và tạo câu trả lời có citation |

Ba workers được dispatch song song bằng LangGraph `Send` API. Kết quả được
ghi vào shared state qua reducer:
...
Giao diện hiển thị theo thời gian thực:

- Supervisor plan.
- Trạng thái ba workers chạy song song.
- Execution log.
- Evidence và warnings của từng worker.
- Tổng số evidence và nguồn.
- Final answer và latency.
- Lựa chọn tổng hợp offline hoặc OpenRouter.
...
Mặc định, Synthesizer chạy offline để bài demo không phụ thuộc API hoặc model
embedding. Muốn dùng OpenRouter cho bước tổng hợp:

```bash
cp Lab_Assignment/.env.example Lab_Assignment/.env
# Điền OPENROUTER_API_KEY
# Đổi SUPERVISOR_USE_LLM=true
```
...
# Nền tảng Day 08 — RAG Pipeline v2
...
├── src/
│   ├── __init__.py
│   ├── supervisor_workers.py  ← Day 09: Supervisor + 3 workers
...
├── requirements.txt
├── run_supervisor.py          ← CLI demo Supervisor-Workers
├── ui/                        ← FastAPI web UI, port 8090
├── tests/
